...
## 版本概述
kiz-lang v0.1.0 是语言的首个正式版本，旨在纪念 2025 年项目启动及核心贡献者的初期付出。**本版本为纪念版，发布后不再提供维护支持**，若发现 Bug 可通过 GitHub Issue 提交，所有问题将在 v0.2.0 版本集中修复。
...
[待实现功能规划（v0.2.0）](##待实现功能规划（v0.2.0）)
[已知局限性（将在v0.2.0优化）](##已知局限性（将在v0.2.0优化）)
## 已支持功能清单
### 1. 核心语法与结构
- 面向对象编程（OOP）支持（原型链模型）
...
| `__next__`         | 迭代器方法（支持 `for` 循环，暂未实现）            |
| `__is_immut__`     | 判断对象可变性（暂未实现，v0.2.0 规划）            |
